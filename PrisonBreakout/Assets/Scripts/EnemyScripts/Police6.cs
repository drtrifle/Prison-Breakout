...
public class Police6 : MonoBehaviour
{
    ...
    void OnCollisionEnter2D(Collision2D coll)
    {
        ...
        if (coll.gameObject.tag == "Wall" && wallTimer <= 0)
        {
            if(changeDirection == new Vector3(1, 1, 0))
            {
                changeDirection = new Vector3(1, -1, 0);
                wallTimer = 5;
            }else if (changeDirection == new Vector3(1, -1, 0))
            {
                changeDirection = new Vector3(-1, -1, 0);
                wallTimer = 5;
            }
